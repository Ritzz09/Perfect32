...
const Services = () => {
  return (
    <section id="services" className="py-10 px-10 md:h-170 md:px-20 relative bg-[#0F172A]"
    style={{
      ...
      <div className="relative z-10 w-full">
        ...
        <div className="relative w-full">
          ...
          >
            {services.map((service, index) => (
              <SwiperSlide key={index}>
                <motion.div
                  className=" rounded-lg overflow-hidden border-yellow-500  bg-gray-300/80 border-4 md:h-107 h-88 m-2 hover:scale-108 hover:border-0"
                ...
                >
                  <img
                    src={service.image}
                    alt={service.title}
                    className="w-full md:h-50 h-40 object-cover border-b-2 border-yellow-500"
                  />
                  
                  <div className="md:p-4 p-2 text-center">
                    <h3 className="md:text-2xl text-xl font-bold text-slate-800">{service.title}</h3>
                    <p className="md:text-lg text-sm text-gray-800 mt-1">{service.description}</p>
                  </div>
                </motion.div>
              </SwiperSlide>
            ))}
          ...
          {/* Navigation Buttons */}
          <div className="absolute top-1/2 md:left-20 left-0  transform -translate-y-1/2 z-10">
            <button className="custom-prev bg-yellow-500 text-slate-800 p-3 rounded-full shadow-lg hover:scale-110 cursor-pointer">
              &#10094;
            </button>
          </div>

          <div className="absolute top-1/2 md:right-20 right-0 transform -translate-y-1/2 z-10">
